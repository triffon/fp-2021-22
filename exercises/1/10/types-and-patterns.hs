...
-- нещо като псевдоним на някой тип.
-- Когато пуснем програмата тези имена изчезват и програмата ни
-- вместо Vector2 ще вижда просто (Int, Int)
...
-- и ще можем да заместим "a" с фиксиран тип
...
-- видове образци:
-- - литерал - съвпада с конкретна стойност (константа)
...
-- Примери:
fact :: Int -> Int
fact 0 = 1  -- получили сме 0 като аргумент и директно връщаме 1
fact n = n * fact (n - 1)
...
-- (все пак те са просто тагове/низове)
