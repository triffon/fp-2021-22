...
(define (deep-repeat-iter l level)
  (cond ((null? l) '())
        ((null? (car l)) (cons '() (deep-repeat-iter (cdr l) level))) ; corner case
        ((pair? (car l)) (cons (deep-repeat-iter (car l) (+ level 1))
                               (deep-repeat-iter (cdr l) level)))
        (else (append (repeat level (car l))
                      (deep-repeat-iter (cdr l) level)))))

(define (deep-repeat l)
  (deep-repeat-iter l 1))
